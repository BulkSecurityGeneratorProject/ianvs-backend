...
export class CompanyService {
    ...
    protected convertDateFromClient(company: ICompany): ICompany {
        const copy: ICompany = Object.assign({}, company, {
            registrationDate:
                company.registrationDate != null && company.registrationDate.isValid() ? company.registrationDate.toJSON() : null
        });
        return copy;
    }
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach((company: ICompany) => {
                company.registrationDate = company.registrationDate != null ? moment(company.registrationDate) : null;
            });
        }
        return res;
    }
